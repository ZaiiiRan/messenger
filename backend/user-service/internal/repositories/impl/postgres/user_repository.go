...
func (r *UserRepository) Query(ctx context.Context, query *models.QueryUsersDal) ([]*user.User, error) {
	...
	appendAnyEqual(&sb, "u.id::text", query.Ids, &args, &argPos)
	appendAnyEqual(&sb, "u.username", query.Usernames, &args, &argPos)
	appendIPrefix(&sb, "u.username", query.PartialUsernames, &args, &argPos)
	appendAnyEqual(&sb, "u.email", query.Emails, &args, &argPos)
	appendIPrefix(&sb, "u.email", query.PartialEmails, &args, &argPos)
	appendAnyEqual(&sb, "p.phone", query.PhoneNumbers, &args, &argPos)
	appendPartialNames(&sb, "p.first_name", "p.last_name", query.PartialNames, &args, &argPos)
