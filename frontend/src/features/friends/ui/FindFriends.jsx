...
const FindFriends = ({ goBack }) => {
    ...
    const loadUsers = async (newSearch = search, newOffset = offset, newEnd = end) => {
        if (newEnd || newSearch.length < 4 || isFetching) return
        setFetching(true)

        const source = axios.CancelToken.source()

        try {
            const response = await shortUsersFetching.fetchShortUser(newSearch, limit, newOffset )
    ...
    return (
        <MainWidget title={ t('Find friends') } goBack={ goBack }>
