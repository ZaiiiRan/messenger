...
const OutgoingFriendRequests = ({ goBack }) => {
    ...
    const loadUsers = async (newSearch = search, newOffset = offset, newEnd = end) => {
        ...
        try {
            const response = await shortUsersFetching.fetchOutgoingFriendRequests(newSearch, limit, newOffset)
    ...
    return (
        <MainWidget title={ t('Outgoing requests') } goBack={ goBack }>
