...
const IncomingFriendRequests = ({ goBack }) => {
    ...
    const loadUsers = async (newSearch = search, newOffset = offset, newEnd = end) => {
        ...
        try {
            const response = await shortUsersFetching.fetchIncomingFriendRequests(newSearch, limit, newOffset)
    ...
    return (
        <MainWidget title={ t('Incoming requests') } goBack={ goBack }>
